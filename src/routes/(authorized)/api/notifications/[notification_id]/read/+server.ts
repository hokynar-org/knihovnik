...
export const POST = (async ({ request, params, locals, url, route }) => {
  ...
  const read_notifications:Notification[] = await db.update(notifications).set({read:true}).where(and(eq(notifications.user_id,user_id),eq(notifications.id,notification_id))).returning();
  if(read_notifications.length == 0) {
    throw error(400);
  }
  return json({notification:read_notifications[0]});
