...
export async function getFileUrl(filename: string) {
    return await getSignedUrl(S3, new GetObjectCommand({ Bucket: BUCKET_NAME, Key: filename }), {
        expiresIn: 3600,
    });
}
...
const allowedFileTypes = new Set(<const>['image/webp', 'image/jpeg', 'image/png', 'image/gif']);
export type FileFormat = typeof allowedFileTypes extends Set<infer T> ? T : never;
